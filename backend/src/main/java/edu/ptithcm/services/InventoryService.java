...
public class InventoryService {
    ...
    private static final BaseMapper<InventoryModel, InventoryInfo> mapper =
        MapperFactory.inventory();

    // Lấy tất cả
    public ResponseDTO<List<InventoryInfo>> getAllInventories()
        throws RuntimeException {
        System.out.println("getAllInventories services before call repo");
        List<InventoryModel> inventories = inventoryRepo.findAll();

        System.out.println(
            "getAllInventories services after call repo : inventories  " +
    ...
    public ResponseDTO<InventoryInfo> createInventory(InventoryRequestDTO req)
        throws RuntimeException {
        if (!req.validForCreate()) return new InvalidResponse<>(
        ...
        if (branch == null) return new NotFoundResponse<>(
            "Chi nhánh không tồn tại"
        );
        if (product == null) return new NotFoundResponse<>(
            "Sản phẩm không tồn tại"
        );

        InventoryModel existing = inventoryRepo.findByBranchAndProduct(
            branch.getId(),
            product.getId()
        );
        if (existing != null) return new InvalidResponse<>(
            "Sản phẩm đã tồn tại trong kho"
        );
    ...
    public ResponseDTO<InventoryInfo> updateInventory(InventoryRequestDTO req)
        throws RuntimeException {
        if (!req.validForUpdate()) return new InvalidResponse<>(
            "Dữ liệu không hợp lệ"
        );

        InventoryModel inventory = inventoryRepo.findById(req.getId());
        if (inventory == null) return new NotFoundResponse<>(
            "Kho không tồn tại"
        );
        ...
        if (req.getBranchId() != null) {
            ...
            if (br == null) return new InvalidResponse<>(
        ...
        if (req.getProductId() != null) {
            ...
            if (p == null) return new InvalidResponse<>(
        ...
        if (inventory.getBranch() != null && inventory.getProduct() != null) {
            List<InventoryModel> branchInventories = inventoryRepo.findByBranch(
                inventory.getBranch().getId()
            );
            if (branchInventories != null) {
                boolean exists = branchInventories
                    .stream()
                    .anyMatch(
                        inv ->
                            inv.getProduct() != null &&
                            inv
                                .getProduct()
                                .getId()
                                .equals(inventory.getProduct().getId()) &&
                            !inv.getId().equals(inventory.getId())
                    );
                if (exists) return new InvalidResponse<>(
                    "Sản phẩm đã tồn tại trong kho của chi nhánh này"
                );
            }
        }
