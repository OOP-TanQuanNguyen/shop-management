...
public class LoyaltyService {
    ...
    private static final CustomerRepository customerRepo =
        Repository.customer();
    private static final BaseMapper<LoyaltyModel, LoyaltyInfo> mapper =
        MapperFactory.loyalty();

    public ResponseDTO<List<LoyaltyInfo>> getAllLoyalty()
        throws RuntimeException {
        return new SuccessResponse<>(
            "Lấy toàn bộ loyalty thành công",
    ...
    public ResponseDTO<LoyaltyInfo> getLoyaltyByCustomer(String customerId)
        ...
        if (loyalty == null) return new NotFoundResponse<>(
            "Không tìm thấy thông tin loyalty"
        );

        return new SuccessResponse<>(
            "Lấy loyalty thành công",
            mapper.toDTO(loyalty)
        );
    }
    ...
    public ResponseDTO<LoyaltyInfo> createLoyalty(String customerId)
        ...
        if (customer == null) return new NotFoundResponse<>(
        ...
        return new SuccessResponse<>(
            "Tạo loyalty thành công",
            mapper.toDTO(loyalty)
        );
    }

    public ResponseDTO<LoyaltyInfo> updateLoyalty(
        String customerId,
        int pointsChange
    ) throws RuntimeException {
        if (
        ...
        LoyaltyModel loyalty = loyaltyRepo.findByCustomerId(customerId);
        if (loyalty == null) return new InvalidResponse<>(
            "Không tìm thấy loyalty của khách hàng"
        );

        int newPoints = loyalty.getTotalPoints() + pointsChange;

        if (newPoints < 0) return new InvalidResponse<>("Điểm không thể âm");

        loyalty.setTotalPoints(newPoints);
        loyalty.setLastUpdate(Timestamp.from(Instant.now()));

        LoyaltyModel updated = loyaltyRepo.update(loyalty);
        if (updated == null) return new NotFoundResponse<>(
            "Không tìm thấy loyalty để cập nhật"
        );

        return new SuccessResponse<>(
            "Cập nhật loyalty thành công",
    ...
    public ResponseDTO<LoyaltyInfo> deleteLoyalty(String customerId)
        ...
        LoyaltyModel loyalty = loyaltyRepo.findByCustomerId(customerId);
        if (loyalty == null) return new SuccessResponse<>(
            "Không có loyalty để xóa",
            null
        );

        LoyaltyModel deleted = loyaltyRepo.delete(loyalty.getId());

        if (deleted == null) return new NotFoundResponse<>(
            "Không tìm thấy loyalty để xóa"
        );

        return new SuccessResponse<>(
            "Xóa loyalty thành công",
